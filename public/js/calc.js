...
function opredelenie() {
  ...
  12 == type && (allw = parseInt(dr_width.value) / 1E3, allh = (parseInt(dr_height.value) + parseInt(fr_height2.value)) / 1E3, S = Sdr + Sf4, stvorki = [0, 0, 0, 0], stv_img = [dr_img, fr1_img], za_prof = (3 * dr_width.value + 2 * dr_height.value) / 1E3, za_prof += (2 * dr_width.value + 2 * fr_height2.value) / 1E3, za_prof *= 380, za_prof += 1E3 * S, cv = 1.45, tip_okna = "<img src='/img/t12.jpg' >");
...
function test_razm() {
  ...
  for (poz_w = 0; poz_w < width.length; poz_w++) {
    width[poz_w].style.color = 400 > width[poz_w].value || 1500 < width[poz_w].value ? "#F00" : "#707070";
  }
...
function test() {
  ...
  if (res) {
    ...
    for (i = 0; i <= stv_img.length; i++) {
      2 == stv_img[i] && (d += 3100 + 1E3 * (stvorki[i] - 1)), 1 == stv_img[i] && (e += 3100 + 1E3 * (stvorki[i] - 1));
    }

    if (4 == type || 5 == type || 10 == type) g = 1 == dr_img ? 2850 : 1800;
    if (9 == type || 11 == type) 1 == n1_img ? (f = -150, cv = 1.75) : f = 0;
    montaz_ceni = [0, 800, 690, 800, 1E3];
    otk_ceni = [[0, 430, 560], [0, 860, 1100], [0, 960, 1250], [0, 1200, 1560], [0, 1600, 2200]];
